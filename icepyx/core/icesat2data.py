...
import icepyx.core.files as files
...
# QUESTION: why doesn't from granules import Granules as Granules work, since
# granules=icepyx.core.granules?
...
class Icesat2Data:
    ...
    def __init__(
        ...
        if files is not None:
            ...
            self._path = files  # TODO: Need to validate

            # self._variables = variables  # TODO: Need to validate
    ...
    @property
    def path(self):
        """Return path to local data file(s)."""

        self._path = Path(self._path).resolve()  # relative -> abs

        return self._path

    @property
    def files(self):
        """Return list of file names from path."""

        if not hasattr(self, "_files"):
            self._files = files.get_file_list(self._path)
    ...
    def order_granules(self, verbose=False, subset=True, email=True, **kwargs):
        ...
        **kwargs : key-value pairs
        ...
        # REFACTOR: add checks here to see if the granules object has been created,and
        # also if it already has a list of avail granules (if not, need to create one
        # and add session)
